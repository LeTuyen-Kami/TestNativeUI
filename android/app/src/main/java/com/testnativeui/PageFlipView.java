...
public class PageFlipView extends GLSurfaceView implements Renderer {
    ...
    public PageFlipView(ReactApplicationContext context) {
        super(context);
        init(context,new ArrayList<>(),new HashMap<>());
    }
    public PageFlipView(ReactApplicationContext context, List<String> ListText, HashMap<String,String> textStyle) {
        super(context);
        init(context,ListText,textStyle);
        mListText = ListText;
        mTextStyle = textStyle;
        mContext = context;
    }

    public PageFlipView(ReactApplicationContext context, AttributeSet attrs) {
        super(context, attrs);
        init(context,new ArrayList<>(),new HashMap<>());
    }
    private void init(ReactApplicationContext context,List<String> mListText,HashMap<String,String> mTextStyle) {
        ...
        mPageRender = new SinglePageRender(context, mPageFlip,
                mHandler, mPageNo,mListText,mTextStyle);
        // configure render
        setRenderer(this);
        ...
        if (mPageFlip.enableAutoPage(enable)) {
            try {
                ...
                if (mPageFlip.getSecondPage() != null &&
                    ...
                    mPageRender = new DoublePagesRender(mContext,
                                                        mPageFlip,
                                                        mHandler,
                                                        mPageNo);
                    ...
                    mPageRender = new SinglePageRender(mContext,
                                                       mPageFlip,
                                                       mHandler,
                                                       mPageNo,
                                                       mListText,
